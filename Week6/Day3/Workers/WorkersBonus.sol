...
- By implementing a new function in the base contract, used by both deletePerson and fire, 
  make sure there is as little code duplication as possible between the deletePerson function 
  and the fire function. 

*/


contract WorkersBonus is People {
   struct workerStruct {
      uint salary;
      address boss;
   }
   mapping(address => workerStruct) workerInfo;

   // Create a worker
   function createWorker(string memory name, uint age, uint height, uint salary, address boss) public {
      require(age <= 75, "Worker age should not be over 75");
      require(boss != msg.sender, "A worker is not allowed to be his/her own boss");

      createPerson(name, age, height);
      workerInfo[msg.sender].salary = salary;
      workerInfo[msg.sender].boss = boss;
   }

   // Fire a worker
   function fireWorker(address boss) public {
      require(boss == workerInfo[msg.sender].boss, "A worker can only be fired by his/her boss");
      deletePerson(msg.sender);
      delete workerInfo[msg.sender];
   }
}